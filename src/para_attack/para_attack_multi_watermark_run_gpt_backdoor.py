...
from src.model.gpt_cls import GPTClassifierConfig, GPTClassifier
from src.model.copier.bert import BertForClassifyWithBackDoor
from trigger.para_attack_base_multiwatermark import BaseTriggerSelector
...
def parse_args():
    ...
    parser.add_argument(
        '--para_attack_file',
        type=str,
        help='File containing paraphrases and their embeddings'
    )

    parser.add_argument(
        '--para_cos_sim_filter',
        type=float,
        default=0.8,  # decided post experiments on different values
        help='Paraphrase emb cosine similarity filter value (minimum)'
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Upto what number of paraphrases used in the attack (i.e., averaging)'
    )
...
def main():
    ...
    processed_datasets = trigger_selector.process_datasets(
        processed_datasets
    )
...
def eval_copier(
    ...
    for i in range(len(target_embs)):
        ...
        AUROC_AUPR_plots(args, i, trigger_results[i], results, accelerator)

        if i == 0:
            results["all"]["pvalue"] = results[i]["pvalue"]
            results["all"]["delta_cos"] = results[i]["delta_cos"]
            results["all"]["delta_l2"] = results[i]["delta_l2"]
            results["all"]["cost_dist_roc_auc"] = results[i]["cost_dist_roc_auc"]
            results["all"]["cost_dist_TPR@0.1FPR"] = results[i]["cost_dist_TPR@0.1FPR"]
            results["all"]["cost_dist_ap"] = results[i]["cost_dist_ap"]
            results["all"]["l2_dist_roc_auc"] = results[i]["l2_dist_roc_auc"]
            results["all"]["l2_dist_TPR@0.1FPR"] = results[i]["l2_dist_TPR@0.1FPR"]
            results["all"]["l2_dist_ap"] = results[i]["l2_dist_ap"]
        else:
            ...
            results["all"]["cost_dist_roc_auc"] = max(results["all"]["cost_dist_roc_auc"], results[i]["cost_dist_roc_auc"])
            results["all"]["cost_dist_TPR@0.1FPR"] = max(results["all"]["cost_dist_TPR@0.1FPR"], results[i]["cost_dist_TPR@0.1FPR"])
            results["all"]["cost_dist_ap"] = max(results["all"]["cost_dist_ap"], results[i]["cost_dist_ap"])
            results["all"]["l2_dist_roc_auc"] = max(results["all"]["l2_dist_roc_auc"], results[i]["l2_dist_roc_auc"])
            results["all"]["l2_dist_TPR@0.1FPR"] = max(results["all"]["l2_dist_TPR@0.1FPR"], results[i]["l2_dist_TPR@0.1FPR"])
            results["all"]["l2_dist_ap"] = max(results["all"]["l2_dist_ap"], results[i]["l2_dist_ap"])
...
def AUROC_AUPR_plots(args, i, trigger_results, results, accelerator):
    watermark_labels = [1] * 20
    watermark_labels.extend([0] * 20)
    cos_dists = list(trigger_results["trigger_cos_dists"].values)
    l2_dists = list(-1 * trigger_results['trigger_l2_dists'].map(lambda x: x[0]).values)  # lower is better
    ...
    results[i]["cost_dist_roc_auc"] = cost_dist_roc_auc
    results[i]["cost_dist_TPR@0.1FPR"] = cost_dist_tpr_at_X_fpr
    fig = plt.figure()
    plt.plot(cost_dist_fpr, cost_dist_tpr, 'b', label='AUC = %0.2f' % cost_dist_roc_auc)
    plt.title(f"Cos. Dist. AUROC")
    plt.savefig(
        f"{args.output_dir}/target-emb-{i}-cos-dist-AUROC.pdf",
    ...
    try:
        l2_dist_tpr_at_X_fpr = l2_dist_tpr[np.where(l2_dist_fpr < 1e-3)[0][-1]]
    except IndexError:
        l2_dist_tpr_at_X_fpr = float("NaN")
    results[i]["l2_dist_roc_auc"] = l2_dist_roc_auc
    results[i]["l2_dist_TPR@0.1FPR"] = l2_dist_tpr_at_X_fpr
    fig = plt.figure()
    plt.plot(l2_dist_fpr, l2_dist_tpr, 'b', label='AUC = %0.2f' % l2_dist_roc_auc)
    plt.title(f"L2 Dist. AUROC")
    plt.savefig(
        f"{args.output_dir}/target-emb-{i}-l2-dist-AUROC.pdf",
        dpi=450, bbox_inches='tight')
    plt.close()
    if args.with_tracking:
    ...
    results[i]["l2_dist_ap"] = l2_dist_ap

    fig = plt.figure()
    plt.plot(l2_dist_p, l2_dist_r, 'b', label='AP = %0.2f' % l2_dist_ap)
    plt.title(f"L2 Dist. AUPR")
    plt.savefig(
        f"{args.output_dir}/target-emb-{i}-l2-dist-AUPR.pdf",
        dpi=450, bbox_inches='tight')
    plt.close()
    if args.with_tracking:
